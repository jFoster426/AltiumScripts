...
Begin
    Result := False;
    If sqrt(((X2 - X1) * (X2 - X1)) + ((Y2 - Y1) * (Y2 - Y1))) < CoordTol Then
    Begin
        Result := True;
    End;
End;
...
Procedure GetNextPrimitive(Primitive : IPCB_Primitive, x : Integer, y : Integer, w : Integer);
Var
...
Begin
    ...
    Begin
        ...
        Begin
            ...
            SpY := Primitive.y2;
        End Else
        If CoordMatch(Primitive.x2, Primitive.y2, x, y) Then
        Begin
            SpX := Primitive.x1;
            SpY := Primitive.y1;
        End Else
    ...
    Begin
        ...
        Begin
            ...
            SpY := Primitive.EndY;
        End Else
        If CoordMatch(Primitive.EndX, Primitive.EndY, x, y) Then
        Begin
        ...
        End;
    End;
    // Bounding box should be W/2 tolerance in any direction to capture touching tracks
    ...
    Begin
        ...
        End Else
        If NewPrimitive.ObjectID = eArcObject Then
        Begin
        ...
        If ((NewPrimitive.Selected = False) And (Abs(NewPrimWidth - W) < 10) And (Coord1Match Or Coord2Match)) Then
        Begin
            NewPrimitive.Selected := True;
            If (NewPrimitive.ObjectID = eTrackObject) Then
            Begin
                GetNextPrimitive(NewPrimitive, SpX, SpY, NewPrimWidth);
            End Else
            If (NewPrimitive.ObjectID = eArcObject) Then
            Begin
                GetNextPrimitive(NewPrimitive, SpX, SpY, NewPrimWidth);
...
Begin
    ...
    Begin
        ...
        GetNextPrimitive(PCBObject, PCBObject.x1, PCBObject.y1, PCBObject.Width);
        GetNextPrimitive(PCBObject, PCBObject.x2, PCBObject.y2, PCBObject.Width);
    End Else
    If PCBObject.ObjectID = eArcObject Then
    Begin
        CoordTol := PCBObject.LineWidth / 2;
        // Run in both directions in case of not closed shape
        GetNextPrimitive(PCBObject, PCBObject.StartX, PCBObject.StartY, PCBObject.LineWidth);
        GetNextPrimitive(PCBObject, PCBObject.EndX, PCBObject.EndY, PCBObject.LineWidth);
    End;
